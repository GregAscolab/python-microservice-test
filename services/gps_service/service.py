...
class GpsService(Microservice):
    ...
    async def _publish_data_recursively(self, base_subject: str, data: dict, timestamp: float):
        ...
        async def publish_datatype(new_subject, value, timestamp):
            try:
            ...
            # Handle lists, but don't publish the whole list as one value
                ...
                for i, item in enumerate(value):
                    if isinstance(item, dict):
                        ...
                        await publish_datatype(f"{new_subject}.{i}", item, timestamp)
            else:
                await publish_datatype(new_subject, value, timestamp)
        ...
        if self.gps and self.use_owa_hardware:
            ...
            if self.gps.gps_pos_ok :
                ...
                payload = {
                    ...
                    "geometry": {
                        "type": "Point",
                        # "coordinates": [self.gps.lastCoord.LonDecimal, self.gps.lastCoord.LatDecimal] # GeoJSON format is a list !
                        "coordinates": {"lon":self.gps.lastCoord.LonDecimal, "lat":self.gps.lastCoord.LatDecimal} # Better using dict to name values (NATS topic)
                    },
                    "properties": {
            ...
            payload = {
                ...
                "geometry": {
                    "type": "Point",
                    # "coordinates": [fake_lon, fake_lat]  # GeoJSON format is a list !
                    "coordinates": {"lon":fake_lon, "lat":fake_lat} # Better using dict to name values in stream name (NATS topic)
                },
                "properties": {
                    "lastCoord": {
                        "PosValid": 1,
